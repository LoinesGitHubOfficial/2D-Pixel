...
// le backround des elements
document.querySelectorAll("span").forEach((elementss) => {
  if (elementss.textContent === "b") {
    if (elementss.id === "n") {
    } else {
      elementss.classList.toggle("stone");
    }
  } else {
    if (elementss.id === "n") {
    } else {
      elementss.style.background = "transparent";
    }
  }
});
